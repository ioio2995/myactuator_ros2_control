...
CallbackReturn MyActuatorHardwareInterface::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  for (const hardware_interface::ComponentInfo & sensor : info_.sensors) {
    can_numbers_[0].emplace_back(std::stoull(sensor.parameters.at("serial_number"), 0, 16));
  }

  for (const hardware_interface::ComponentInfo & joint : info_.joints) {
    can_numbers_[1].emplace_back(std::stoull(joint.parameters.at("serial_number"), 0, 16));
  ...
  myactuator = new MyActuatorCAN();
  CHECK_TS(myactuator->init(can_numbers_));

  for (size_t i = 0; i < info_.joints.size(); i++) {
    float torque_constant;
    CHECK_TS(myactuator->read(
      can_numbers_[1][i], AXIS__MOTOR__CONFIG__TORQUE_CONSTANT + per_axis_offset * axes_[i],
      torque_constant));
    torque_constants_.emplace_back(torque_constant);

    if (enable_watchdogs_[i]) {
      CHECK_TS(myactuator->write(
        can_numbers_[1][i], AXIS__CONFIG__WATCHDOG_TIMEOUT + per_axis_offset * axes_[i],
        std::stof(info_.joints[i].parameters.at("watchdog_timeout"))));
    }
    CHECK_TS(myactuator->write(
      can_numbers_[1][i], AXIS__CONFIG__ENABLE_WATCHDOG + per_axis_offset * axes_[i],
...
CallbackReturn MyActuatorHardwareInterface::on_activate(const rclcpp_lifecycle::State &)
{
  for (size_t i = 0; i < info_.joints.size(); i++) {
    if (enable_watchdogs_[i]) {
      CHECK_TS(
        myactuator->call(can_numbers_[1][i], AXIS__WATCHDOG_FEED + per_axis_offset * axes_[i]));
    }
    CHECK_TS(myactuator->call(can_numbers_[1][i], CLEAR_ERRORS));
  }
...
CallbackReturn MyActuatorHardwareInterface::on_deactivate(const rclcpp_lifecycle::State &)
{
  ...
  for (size_t i = 0; i < info_.joints.size(); i++) {
    CHECK_TS(myactuator->write(
      can_numbers_[1][i], AXIS__REQUESTED_STATE + per_axis_offset * axes_[i], requested_state));
  }
...
return_type MyActuatorHardwareInterface::prepare_command_mode_switch(
...
return_type MyActuatorHardwareInterface::perform_command_mode_switch(
  ...
  for (size_t i = 0; i < info_.joints.size(); i++) {
    ...
    switch (control_level_[i]) {
      case integration_level_t::UNDEFINED:
        ...
        CHECK_RW(myactuator->write(
          can_numbers_[1][i], AXIS__REQUESTED_STATE + per_axis_offset * axes_[i],
          requested_state));
        break;
      ...
      case integration_level_t::EFFORT:
        ...
        CHECK_RW(myactuator->write(
          can_numbers_[1][i], AXIS__CONTROLLER__INPUT_TORQUE + per_axis_offset * axes_[i],
          input_torque));
      ...
      case integration_level_t::VELOCITY:
        ...
        CHECK_RW(myactuator->write(
          can_numbers_[1][i], AXIS__CONTROLLER__INPUT_VEL + per_axis_offset * axes_[i],
          input_vel));
        input_torque = hw_commands_efforts_[i];
        CHECK_RW(myactuator->write(
          can_numbers_[1][i], AXIS__CONTROLLER__INPUT_TORQUE + per_axis_offset * axes_[i],
      ...
      case integration_level_t::POSITION:
        ...
        CHECK_RW(myactuator->write(
          can_numbers_[1][i], AXIS__CONTROLLER__INPUT_POS + per_axis_offset * axes_[i],
          input_pos));
        input_vel = hw_commands_velocities_[i] / 2 / M_PI;
        CHECK_RW(myactuator->write(
          can_numbers_[1][i], AXIS__CONTROLLER__INPUT_VEL + per_axis_offset * axes_[i],
...
return_type MyActuatorHardwareInterface::read(const rclcpp::Time &, const rclcpp::Duration &)
{
  for (size_t i = 0; i < info_.sensors.size(); i++) {
    float vbus_voltage;

    CHECK_RW(myactuator->read(can_numbers_[0][i], VBUS_VOLTAGE, vbus_voltage));
  ...
  for (size_t i = 0; i < info_.joints.size(); i++) {
    ...
    CHECK_RW(myactuator->read(
      can_numbers_[1][i], AXIS__MOTOR__CURRENT_CONTROL__IQ_MEASURED + per_axis_offset * axes_[i],
      Iq_measured));
    hw_efforts_[i] = Iq_measured * torque_constants_[i];

    CHECK_RW(myactuator->read(
      can_numbers_[1][i], AXIS__ENCODER__VEL_ESTIMATE + per_axis_offset * axes_[i],
      vel_estimate));
    hw_velocities_[i] = vel_estimate * 2 * M_PI;

    CHECK_RW(myactuator->read(
      can_numbers_[1][i], AXIS__ENCODER__POS_ESTIMATE + per_axis_offset * axes_[i],
      pos_estimate));
    hw_positions_[i] = pos_estimate * 2 * M_PI;

    CHECK_RW(
      myactuator->read(can_numbers_[1][i], AXIS__ERROR + per_axis_offset * axes_[i], axis_error));
    hw_axis_errors_[i] = axis_error;

    CHECK_RW(myactuator->read(
      can_numbers_[1][i], AXIS__MOTOR__ERROR + per_axis_offset * axes_[i], motor_error));
    hw_motor_errors_[i] = motor_error;

    CHECK_RW(myactuator->read(
      can_numbers_[1][i], AXIS__ENCODER__ERROR + per_axis_offset * axes_[i], encoder_error));
    hw_encoder_errors_[i] = encoder_error;

    CHECK_RW(myactuator->read(
      can_numbers_[1][i], AXIS__CONTROLLER__ERROR + per_axis_offset * axes_[i],
      controller_error));
    hw_controller_errors_[i] = controller_error;

    CHECK_RW(myactuator->read(
      can_numbers_[1][i], AXIS__MOTOR__FET_THERMISTOR__TEMPERATURE + per_axis_offset * axes_[i],
      fet_temperature));
...
return_type MyActuatorHardwareInterface::write(const rclcpp::Time &, const rclcpp::Duration &)
{
  for (size_t i = 0; i < info_.joints.size(); i++) {
    float input_torque, input_vel, input_pos;
