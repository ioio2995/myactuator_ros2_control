...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MYACTUATOR_BROADCASTER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MYACTUATOR_BROADCASTER_HPP_
...
#include "myactuator_broadcaster/myactuator_broadcaster.hpp"
...
using ControllerStateMsg = myactuator_broadcaster::MyactuatorBroadcaster::ControllerStateMsg;
using ControllerReferenceMsg = myactuator_broadcaster::MyactuatorBroadcaster::ControllerReferenceMsg;
using ControllerModeSrvType = myactuator_broadcaster::MyactuatorBroadcaster::ControllerModeSrvType;
...
class TestableMyactuatorBroadcaster : public myactuator_broadcaster::MyactuatorBroadcaster
{
  FRIEND_TEST(MyactuatorBroadcasterTest, all_parameters_set_configure_success);
  FRIEND_TEST(MyactuatorBroadcasterTest, activate_success);
  FRIEND_TEST(MyactuatorBroadcasterTest, reactivate_success);
  FRIEND_TEST(MyactuatorBroadcasterTest, test_setting_slow_mode_service);
  FRIEND_TEST(MyactuatorBroadcasterTest, test_update_logic_fast);
  FRIEND_TEST(MyactuatorBroadcasterTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = myactuator_broadcaster::MyactuatorBroadcaster::on_configure(previous_state);
...
class MyactuatorBroadcasterFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_myactuator_broadcaster/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_myactuator_broadcaster/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_myactuator_broadcaster")
  {
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_myactuator_broadcaster/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableMyactuatorBroadcaster> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MYACTUATOR_BROADCASTER_HPP_
